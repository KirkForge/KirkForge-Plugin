...
import { doctor, buildCorrectionPrompt, recordObservation, recallRoutingBias, verifyWorkspace } from "@55ndeep/plugin";
import type { ReducedStatePacket } from "@55ndeep/plugin";
...
program
  ...
  .action(async (description, opts) => {
    ...
    const { orchestrator, shutdown } = await createBootstrap(opts);
    ...
    if (opts.json) {
      ...
      if (result.ok) {
        ...
        if (result.value.packet) {
          ...
          console.log(`\n--- Verification ---`);
          console.log(`  Lint:  ${p.verification.lint.errors} errors, ${p.verification.lint.warnings} warnings`);
          console.log(`  Types: ${p.verification.types.errors} errors`);
          console.log(`  Security: ${p.verification.security.findings} findings (${p.verification.security.critical} critical)`);
...
program
  ...
  .option("--context <text>", "Additional context for the task")
  .option("--file <paths...>", "Target files for the task")
  .option("--validator <command>", "Structured validator: command name (no shell expansion, args passed separately)")
  .option("--validator-args <args...>", "Arguments for structured --validator")
  .option("--validator-shell <command>", "Raw shell validator command (unsafe: host must sandbox); exit 0 means pass")
  ...
  .action(async (description, opts) => {
    ...
    if (opts.maxCost !== undefined && maxCost !== undefined && (Number.isNaN(maxCost) || maxCost < 0)) {
    ...
    if (opts.json) {
      console.log(JSON.stringify({
        ...
        taskPass: outcome.taskValidation.status === "pass" ? true : outcome.taskValidation.status === "fail" ? false : null,
      ...
      for (let i = 0; i < outcome.turns.length; i++) {
        ...
        console.log(`  Turn ${i + 1}: ${t.action} — ${t.rationale} [${t.workerTokens} tokens, ${t.packet.verification.overall}]`);
      }
      ...
      if (outcome.sourceOfTruth === "task-validator") {
        console.log(`Task validator: ${outcome.taskValidation.status} — ${outcome.taskValidation.reason ?? outcome.taskValidation.validator}`);
      }
    ...
    if (opts.execute) {
      ...
      if (execResult.ok) {
        ...
        if (opts.json) {
          ...
          console.log("Execution complete: " + er.succeededCount + "/" + er.totalSubtasks + " subtasks succeeded");
          console.log("Total tokens: " + er.totalTokens + " | Duration: " + (er.totalDurationMs / 1000).toFixed(1) + "s\n");
          for (const r of er.results) {
            const status = r.ok ? "✓" : "✗";
            console.log("  " + status + " [" + r.nodeId + "] " + r.description.slice(0, 60) + " (" + (r.durationMs / 1000).toFixed(1) + "s, " + r.tokensUsed + " tokens)");
...
program.command("verify")
  ...
  .action(async (opts) => {
    ...
    if (opts.json) {
      ...
      console.log(`  Security:       ${packet.verification.security.findings} findings (${packet.verification.security.critical} critical, ${packet.verification.security.high} high)`);
...
program
  ...
  .action(async (opts) => {
    ...
    if (opts.pretty) {
      ...
      const tools: [string, { available: boolean; version?: string; source?: string; note?: string }][] = [
      ...
      for (const [name, cap] of tools) {
        ...
        const status = cap.available ? `available (${cap.version ?? "bundled"})${src}` : `not found${src}`;
...
program
  ...
  .action(async () => {
    ...
    console.log(`EventBus:       ${h.eventBus.running ? "running" : "stopped"} (inflight: ${h.eventBus.inflight})`);
    ...
    if (slo) {
      ...
      for (const w of slo.windows) {
        ...
        console.log(`  ${w.name}: rate=${pct}% budget=${budgetPct}% burn=${w.burnRate.toFixed(2)}x status=${w.status}`);
      }
    } else {
