...
This is a deterministic local validator example. It is **not a benchmark claim**. It demonstrates how a host CLI can use 55NDeep's verifier battery and a local test runner to produce `taskPass: true` / `taskPass: false` evidence.
...
The validator runs `ruff check` (lint) and `pyright` (types) via 55NDeep, then runs `pytest` as a local task validator.
...
examples/validator-task/
  ...
  run-validator.sh     # Run 55ndeep verify-workspace + local pytest
  README.md            # This file
```
...
55ndeep verify-workspace --workspace examples/validator-task --language python
...
55ndeep observe --memory /tmp/validator-task-mem.json \
