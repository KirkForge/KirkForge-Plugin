...
export async function chatCompletion(messages: ChatMessage[], options: ModelClientOptions, headers: Record<string, string> = {}): Promise<ModelResponse> {
  ...
  try {
    ...
    return parseChatCompletionResponse(await res.json(), ChatCompletionSchema, options, ["reasoning"]);
  } catch (e) {
    if (e instanceof ModelClientError) throw e;
    if (e instanceof DOMException && e.name === "AbortError") throw ModelClientError.timeout(options.timeoutMs);
...
export async function anthropicCompletion(messages: ChatMessage[], options: ModelClientOptions): Promise<ModelResponse> {
