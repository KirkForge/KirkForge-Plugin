...
function tryResolveExtension(basePath: string): string | null {
  ...
  for (const ext of [".ts", ".tsx", ".js"]) {
    if (existsSync(resolve(dirname(basePath), "index" + ext))) return resolve(dirname(basePath), "index" + ext);
  }
  return null;
}
...
export class GraphifyEmitter {
  ...
  async emit(taskId: string): Promise<Result<GraphifyReport, Error>> {
    ...
    try {
      ...
      if (targets.length === 0) {
        const report: GraphifyReport = { taskId, edgeCount: 0, newEdges: 0, brokenEdges: 0, cycles: 0, durationMs: 0 };
        await eventBus?.emit({
          kind: "state.graph",
          schemaVersion: "v3",
          sequence: 0,
          streamId: taskId,
          taskId,
          value: { status: "skipped", edgeCount: 0, newEdges: 0, brokenEdges: 0, cycles: 0, durationMs: 0 },
      ...
      for (const f of targets) {
        ...
        for (const line of src.split("\n")) {
          ...
          if (m) {
            const kind = line.includes("import type") ? "type-only" as const : "import" as const;
            const resolved = resolveImport(m[1]!, f, cwd);
            const key = `${f}|${resolved}`;
            if (!seen.has(key)) { seen.add(key); edges.push({ from: f, to: resolved, kind }); }
          }
          // require() calls
          ...
          if (req) {
            ...
            const key = `${f}|${resolved}`;
            if (!seen.has(key)) { seen.add(key); edges.push({ from: f, to: resolved, kind: "import" }); }
          }
          // export ... from "..."
          ...
          if (dyn) {
            ...
            const key = `${f}|${resolved}`;
            if (!seen.has(key)) { seen.add(key); edges.push({ from: f, to: resolved, kind: "dynamic" }); }
          }
        }
      }
      ...
      const report: GraphifyReport = { taskId, edgeCount: edges.length, newEdges: edges.length, brokenEdges, cycles, durationMs: Date.now() - start };
      ...
      await eventBus?.emit({
        ...
        streamId: taskId,
        taskId,
        value: { status, edgeCount: edges.length, newEdges: edges.length, brokenEdges, cycles, durationMs: report.durationMs },
        timestamp: new Date().toISOString(),
      });
      ...
      const report: GraphifyReport = { taskId, edgeCount: 0, newEdges: 0, brokenEdges: 1, cycles: 0, durationMs: Date.now() - start };
      await eventBus?.emit({
        kind: "state.graph",
        schemaVersion: "v3",
        sequence: 0,
        streamId: taskId,
        taskId,
        value: { status: "error", error: message, edgeCount: 0, newEdges: 0, brokenEdges: 1, cycles: 0, durationMs: report.durationMs },
        timestamp: new Date().toISOString(),
      });
      return ok(report);
    }
