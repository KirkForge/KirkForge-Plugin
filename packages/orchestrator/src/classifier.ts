...
const MODE_SCORING: Array<{ pattern: RegExp; mode: DelegationMode; score: number; reason: string }> = [
  // artifact wins strongly for explicit file-creation language
  { pattern: /\b(?:generate|create|write|build|make)\s+(?:a\s+)?(?:\w+\s+)?(?:file|component|module|service|class|server|app|script)\b/i, score: 20, mode: "artifact", reason: "file creation task" },
  { pattern: /\b(?:file|files|write to|save to)\b/i, score: 10, mode: "artifact", reason: "file output task" },
  // schema-contract for structured/audit work
  { pattern: /\b(?:structured response|json schema|contract format|audit report)\b/i, score: 15, mode: "schema-contract", reason: "structured/contract task" },
  { pattern: /\b(?:audit|assess|evaluate|review\s+(?:the|this))\b/i, score: 8, mode: "schema-contract", reason: "analysis/audit task" },
  { pattern: /\b(?:validate|verify)\b/i, score: 5, mode: "schema-contract", reason: "validation task" },
  // hard-prompt for repairs/fixes
  // task-decompose for multi-step/pipeline work
  { pattern: /\b(?:full-stack|end-to-end|multi-step|pipeline|workflow|build a (?:complete|full|whole)|from scratch|boilerplate|scaffold)\b/i, score: 25, mode: "task-decompose", reason: "multi-step/pipeline task" },
  { pattern: /\b(?:break (?:down|into|this)|decompose|subtasks|step (?:by step|1)|plan (?:out|the))\b/i, score: 20, mode: "task-decompose", reason: "explicit decomposition request" },
  { pattern: /\b(?:fix|lint error|repair|refactor)\b/i, score: 5, mode: "hard-prompt", reason: "repair/fix task" },
];

function classifyByScoring(description: string): { mode: DelegationMode; reason: string; confidence: number } {
  ...
  for (const m of ["artifact", "schema-contract", "hard-prompt", "task-decompose"] as const) {
    if ((scores[m] ?? 0) > highest) { best = m; highest = scores[m]!; }
  }
...
export function classifyTask(task: TaskInput, classifierMemory?: ClassifierMemory | null): DelegationDecision {
  ...
  if (regexResult.confidence >= NLP_FALLBACK_THRESHOLD) {
    return { mode: regexResult.mode, reason: regexResult.reason, autoRouted: regexResult.mode !== "hard-prompt" || regexResult.confidence > 0.1 };
  }
